...
fractale::fractale(unsigned int width, unsigned int height) : fractale() {
    this->width = width;
    this->height = height;
}

fractale::fractale(unsigned int width, unsigned int height, double left, double right, double top, double bottom) : fractale(width, height){
    this->left = left;
    this->right = right;
    this->top = top;
    this->bottom = bottom;
...
std::complex<double> fractale::scale(unsigned int x, unsigned int y) {
    double re = (right - left)*double(x)/double(width) + left;
    double im = -((top - bottom)*double(y)/double(height) + bottom);
    return re + im*1i;
}

void fractale::zoom(unsigned int width, unsigned int height, int startx, int starty, int endx, int endy) {
    this->width = width;
    this->height = height;
